...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        B, D, Hp, Wp = z.shape
        seq = z.flatten(2).transpose(1, 2)  # (B, N, D)
        return seq, (Hp, Wp)
...
class PatchUnembed(nn.Module):
    ...
    def forward(self, seq, hw):
        Hp, Wp = hw
        B, N, D = seq.shape
        z = seq.transpose(1, 2).reshape(B, D, Hp, Wp)  # (B, D, Hp, Wp)
        x_rec = self.deproj(z)  # (B, out_ch, H, W)
        return x_rec

#Original Conv block with BatchNorm
# class Conv(nn.Module):
#     def __init__(self, h_dim, head, dropout=0.0):
#         super().__init__()
#         self.conv = GATv2Conv(h_dim, h_dim // head, heads=head,concat=True,add_self_loops=False,dropout=dropout)
...
#Attention and feedforward block with LayerNorm
class Conv(nn.Module):
...
#         self.attn = GATv2Conv(
#             h_dim, h_dim // head,
#             heads=head, concat=True,
#             add_self_loops=False, dropout=dropout
#         )

#         self.norm1 = GraphNorm(h_dim)
#         self.drop1 = nn.Dropout(dropout)

#         self.ffn = nn.Sequential(
#             nn.Linear(h_dim, ffn_mult * h_dim),
#             nn.GELU(),
#             nn.Dropout(dropout),
#             nn.Linear(ffn_mult * h_dim, h_dim),
#         )
#         self.norm2 = GraphNorm(h_dim)
#         self.drop2 = nn.Dropout(dropout)

#     def forward(self, x, edge_index, batch):
#         # PreNorm attention
#         h = self.attn(self.norm1(x, batch), edge_index)
#         x = x + self.drop1(h)

#         # PreNorm FFN
#         h = self.ffn(self.norm2(x, batch))
#         x = x + self.drop2(h)
#         return x
...
class Block(nn.Module):
    def __init__(self, in_channel, h_dim, patch_size, head, do_pool=True, dropout=0.0):
        ...
        self._printed = False  # debug print once
    ...
    def forward(self, x):
        ...
        #edge_index =neighbor0_4(Hp)
        #edge_index = neighbor0_4(Hp) if self.patch_size == 1 else neighbor0(Hp)
    ...
    #     # ✅ pass g.batch for GraphNorm
    #     z = self.conv1(g.x, g.edge_index, g.batch)
    #     z = self.conv2(z, g.edge_index, g.batch)
...
#     """
#     2-cell branch computed directly from the RAW image x_raw using:
#       - conv2d(kernel=2p, stride=p) => features on (Hp-1)x(Wp-1) "square" grid
#       - GNN message passing on that 2-cell grid
#       - reshape back to image-like tensor
#     """
#     def __init__(self, in_channel, h_dim, patch_size, head, dropout=0.0):
#         super().__init__()
#         self.patch_size = patch_size
#         # each output location corresponds to a 2x2 block of patches (a square)
...
#         # x_raw: (B, C, H, W)
#         z2 = self.cell2(x_raw)            # (B, h_dim, H2, W2)  typically H2=Hp-1
#         B, D, H2, W2 = z2.shape
#         seq2 = z2.flatten(2).transpose(1, 2)  # (B, N2, D), N2=H2*W2

#         edge_index2 = neighbor0(H2)       # adjacency on the 2-cell grid
...
#         out = out.reshape(B, H2, W2, D).permute(0, 3, 1, 2).contiguous()  # (B, D, H2, W2)
#         return out

# #Original TwoCellBlock with BatchNorm
# class TwoCellBlock(nn.Module):
...
#         #self.cell2 = nn.Conv2d(in_channel, h_dim,kernel_size=2 * patch_size,stride=patch_size,bias=True)
#         self.cell2 = nn.Conv2d(in_channel, h_dim,kernel_size=2 * patch_size,stride=patch_size,padding=patch_size // 2, bias=True)

#         self.pre_norm = nn.GroupNorm(num_groups=8, num_channels=h_dim)
...
#     def forward(self, x_raw):
#         z2 = self.cell2(x_raw)                      # (B, h_dim, H2, W2)
#         z2 = self.pre_act(self.pre_norm(z2))        # stabilize

#         B, D, H2, W2 = z2.shape
#         seq2 = z2.flatten(2).transpose(1, 2)        # (B, N2, D)

#         edge_index2 = neighbor0_4(H2)               # 4-neighbor on 2-cell grid
#         g2 = self.make_batch_graph(seq2, edge_index2, add_loop=False)
...
#TwoCellBlock with POSITIONAL ENCODING
class TwoCellBlock(nn.Module):
    def __init__(self, in_channel, h_dim, patch_size, head, dropout=0.0, use_two_convs=False):
        super().__init__()
        self.patch_size = patch_size
        ...
        self.pre_act  = nn.GELU()
        # ✅ NEW: positional embedding for (row, col) on the 2-cell grid
    ...
    def forward(self, x_raw):
        ...
        B, D, H2, W2 = z2.shape
        seq2 = z2.flatten(2).transpose(1, 2)        # (B, N2, D) where N2=H2*W2

        # ✅ NEW: add (row, col) positional encoding
        device = seq2.device
        coords = torch.stack(torch.meshgrid(torch.arange(H2, device=device),torch.arange(W2, device=device),indexing="ij"),dim=-1).reshape(-1, 2).float()  # (N2, 2)
        ...
        pos2 = self.pos_proj2(coords)               # (N2, h_dim)
        seq2 = seq2 + pos2.unsqueeze(0)             # (B, N2, h_dim)

        # 4-neighbor graph on 2-cell grid (von Neumann)
...
#         out = self.conv1(g2.x, g2.edge_index,g2.batch)
#         if self.use_two_convs:
#             out = self.conv2(out, g2.edge_index,g2.batch)
...
class CCNN(nn.Module):
    def __init__(self, in_channel, h_dim, head, class_num, layer=7, dropout=0.0):
        super().__init__()
        # self.patch = [2, 2, 1, 1] #Patch sizes for each layer
        # self.in_dim = [in_channel, h_dim, h_dim, h_dim, h_dim]

        self.patch = [2, 2, 1, 1, 1,1,1]#Patch sizes for each layer
        self.in_dim = [in_channel, h_dim, h_dim, h_dim, h_dim, h_dim,h_dim,h_dim]
        ...
        #self.fc1 = nn.Linear(2*h_dim, h_dim)
        #self.fc2 = nn.Linear(h_dim, class_num)
        
        # self.fuse_gate = nn.Sequential(nn.Linear(2 * h_dim, h_dim),nn.Sigmoid())
        # self.fuse_proj = nn.Sequential(nn.Linear(h_dim, h_dim), nn.ReLU(), nn.Dropout(dropout))
    ...
    def forward(self, x):
        ...
        x2 = self.two_cell(x_raw)  # (B, h_dim, H2, W2)
        #x2 = self.pool(x2).flatten(1)  # (B, h_dim)
        
        #Experiment A: pool after last block
        for layer in self.layers:
            x = layer(x)
        
        # # Experiment B: pool after block 2
        # for i, layer in enumerate(self.layers):
        #     x = layer(x)
        #     if i in [1]:     # <-- POOL after block 2
        #         x = self.maxpool(x)
        ...
        g = self.fuse_gate2d(torch.cat([x0, x2], dim=1))  # (B, h_dim, H0, W0)
        
        
        #x0 = self.pool(x).flatten(1)  # (B, h_dim)
        ...
        x = self.fc2(xf)
        # x = torch.cat([x0, x2], dim=1)  # (B, 2*h_dim)
